...
export const buildComparisonSeries = (
  _,
  indicatorKey,
  localSelectedValues,
  selectedColorMap,
  ...
  const indicatorData = detailedData.comparaciones[activeThematicKey][indicatorKey];

  if (!indicatorData) {
    return { data: [], series: [] };
  }

  // Sin límite de cantidad de series
  const selected = localSelectedValues || [];

  const series = selected.map((val, idx) => ({
    key:   sanitizeKey(String(val)),
    label: String(val),
    color: selectedColorMap?.get(val) || COMPARE_COLORS[idx % COMPARE_COLORS.length] || COMPARE_COLORS[0],
    raw:   val,
  }));
  ...
  if (indicatorData.grupos) {
    const data = indicatorData.grupos.map((grupoObj) => {
      const label =
        indicatorKey === 22 || indicatorKey === 39
          ? "Estrato " + grupoObj.grupo
          : grupoObj.grupo;
      const row = { label };
      ...
      series.forEach((s) => {
        ...
        let value = found?.value ?? found?.valor ?? 0;
...
export const buildPopulationComparisonSeries = (
  ...
  const data = populationIds.map((id, index) => {
    const indicator = detailedData.comparaciones[activeThematicKey][id];
